...
const CATEGORIES = [
  { name: "Resuscitation", icon: Activity, color: "from-[#0891B2] to-[#22D3EE]" },
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-6 py-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#0891B2]/10 border border-[#0891B2]/20">
            <Sparkles className="w-4 h-4 text-[#0891B2]" />
            <span className="text-sm font-medium text-[#0891B2]">AI-Powered Search</span>
          </div>
          ...
          <p className="text-sm text-[#5E6B6A]">
            Press <kbd className="px-2 py-1 rounded bg-white border border-[#99F6E4] text-xs font-mono">Enter</kbd> to ask AI or type to search
          </p>
        </div>
          ...
          <div className="animate-fade-in">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[#134E4A] flex items-center gap-2">
                Search Results
                <Badge variant="secondary" className="bg-[#0891B2]/10 text-[#0891B2] border-[#0891B2]/20">
                  live
                </Badge>
              </h2>
              <Button
          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                {CATEGORIES.map((cat) => {
                  const count = allGuidelines?.filter(
                  ...
                  return (
                    ...
                    >
                      <Card className="p-4 hover:shadow-lg hover:shadow-[#0891B2]/5 hover:border-[#0891B2]/30 transition-all duration-200 cursor-pointer h-full">
                        ...
                        <p className="font-semibold text-[#134E4A] text-sm">{cat.name}</p>
                        <p className="text-xs text-[#5E6B6A] mt-1">{count} guidelines</p>
                      </Card>
                    </Link>
                  );
                })}
              ...
              <div className="space-y-3">
                ...
                {allGuidelines?.length === 0 && (
                  <Card className="p-8 text-center">
                    <p className="text-[#5E6B6A]">No guidelines yet. Check back soon.</p>
                  </Card>
                )}
              </div>
            </div>
