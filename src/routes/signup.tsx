    ...
    >
      ...
      >
        ...
        <div className="relative">
          <motion.div
            className="w-16 h-16 rounded-xl bg-gradient-to-br from-accent via-primary to-secondary flex items-center justify-center shadow-lg shadow-primary/30"
            animate={{
              scale: [1, 1.05, 1],
            }}
            transition={{
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center p-3 sm:p-4 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        />
        <motion.div
          className="absolute top-1/4 left-0 w-56 h-56 rounded-full opacity-10"
          style={{
            background: 'radial-gradient(circle, oklch(0.65 0.15 155 / 0.3) 0%, transparent 60%)',
            filter: 'blur(40px)',
          }}
          animate={{
            x: [0, 30, 0],
      ...
      <Card className="relative z-10 w-full max-w-md backdrop-blur-sm bg-card/98 overflow-hidden">
        <AnimatePresence>
          {isAnyLoading && (
            <LoadingOverlay 
              message={isLoading ? "Creating account" : isMsLoading ? "Connecting to Microsoft" : "Connecting to Google"} 
            />
          )}
        </AnimatePresence>
